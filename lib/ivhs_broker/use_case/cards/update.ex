defmodule IvhsBroker.UseCase.Cards.Update do
  ...
  def validate({card_uid, params}, _options) do
    with {:ok, %Card{} = card} <- Cards.get_card(card_uid),
         %Ecto.Changeset{valid?: true} = changeset <- Card.update_changeset(card, params) do
      {:ok, {card, changeset}}
    else
      {:error, _} = error ->
        error
  ...
  def after_run(%{card: card}, options) do
    ...
    notify(options, type: :success, message: gettext("Card updated successfully"))
  end
end
